...
@RoutePage(name: 'StoragePermissionScreenRoute')
class StoragePermissionScreen extends StatefulWidget {
  const StoragePermissionScreen({super.key, this.next, this.nextPage});

  @override
  State<StoragePermissionScreen> createState() =>
...
class _StoragePermissionScreenState extends State<StoragePermissionScreen> {
  ...
  void initState() {
    super.initState();
    context.read<PermissionBloc>().add(PermissionEventCheckStoragePermission());
  }

  Future<void> _handlePermanentlyDenied() async {
    context.showConfirmationDialog(
      title: LocaleKeys
          .core_common_features_permission_presentation_screens_storage_permission_screen_dialog_reAskTitle,
      message: LocaleKeys
          .core_common_features_permission_presentation_screens_storage_permission_screen_dialog_reAskMessage,
  ...
  void _onContinuePressed() {
    context.read<PermissionBloc>().add(
      PermissionEventRequestStoragePermission(),
    );
  }

  void _onSkipPressed() {
    context.read<PermissionBloc>().add(PermissionEventSkipStoragePermission());
  }
    ...
    return BlocConsumer<PermissionBloc, PermissionState>(
      listenWhen: (previous, current) {
        ...
        final bool statusChanged =
            previous.storagePermissionStatus != current.storagePermissionStatus;
        final bool skippedChanged =
            previous.skippedStoragePermissionGranted !=
            current.skippedStoragePermissionGranted;
        ...
        final bool retryDenied =
            current.storagePermissionStatus ==
                PermissionStatus.permanentlyDenied &&
            previous.storagePermissionStatus ==
                PermissionStatus.permanentlyDenied &&
      ...
      listener: (context, state) async {
        ...
        if (state.storagePermissionStatus == PermissionStatus.granted ||
            state.skippedStoragePermissionGranted) {
        ...
        if (state.storagePermissionStatus ==
            PermissionStatus.permanentlyDenied) {
          ...
          body: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                AppText(
                  LocaleKeys
                      .core_common_features_permission_presentation_screens_storage_permission_screen_title,
                ...
                Padding(
                  ...
                  child: AppText(
                    LocaleKeys
                        .core_common_features_permission_presentation_screens_storage_permission_screen_message,
