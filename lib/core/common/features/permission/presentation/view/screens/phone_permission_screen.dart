...
@RoutePage(name: 'PhonePermissionScreenRoute')
class PhonePermissionScreen extends StatefulWidget {
  const PhonePermissionScreen({super.key, this.next, this.nextPage});
...
class _PhonePermissionScreenState extends State<PhonePermissionScreen> {
  ...
  void initState() {
    super.initState();
    context.read<PermissionBloc>().add(PermissionEventCheckPhonePermission());
  }

  Future<void> _handlePermanentlyDenied() async {
    context.showConfirmationDialog(
      title: LocaleKeys
          .core_common_features_permission_presentation_screens_phone_permission_screen_dialog_reAskTitle,
      message: LocaleKeys
          .core_common_features_permission_presentation_screens_phone_permission_screen_dialog_reAskMessage,
  ...
  void _onContinuePressed() {
    context
        .read<PermissionBloc>()
        .add(PermissionEventRequestPhonePermission());
  }

  void _onSkipPressed() {
    context.read<PermissionBloc>().add(PermissionEventSkipPhonePermission());
  }
    ...
    return BlocConsumer<PermissionBloc, PermissionState>(
      listenWhen: (previous, current) {
        final bool statusChanged =
            previous.phonePermissionStatus != current.phonePermissionStatus;
        final bool skippedChanged =
            previous.skippedPhonePermissionGranted !=
            current.skippedPhonePermissionGranted;
        final bool retryDenied =
            current.phonePermissionStatus ==
                PermissionStatus.permanentlyDenied &&
            previous.phonePermissionStatus ==
                PermissionStatus.permanentlyDenied &&
            previous.isLoading &&
            !current.isLoading;
      ...
      listener: (context, state) async {
        ...
        if (state.phonePermissionStatus == PermissionStatus.granted ||
            state.skippedPhonePermissionGranted) {
        ...
        AppLogger.debug(
          'Phone permission status: ${state.phonePermissionStatus}',
        );

        if (state.phonePermissionStatus == PermissionStatus.permanentlyDenied) {
          ...
          body: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                AppText(
                  LocaleKeys
                      .core_common_features_permission_presentation_screens_phone_permission_screen_title,
                ...
                Padding(
                  ...
                  child: AppText(
                    LocaleKeys
                        .core_common_features_permission_presentation_screens_phone_permission_screen_message,
