...
@RoutePage(name: 'BluetoothPermissionScreenRoute')
class BluetoothPermissionScreen extends StatefulWidget {
  const BluetoothPermissionScreen({super.key, this.next, this.nextPage});

  @override
  State<BluetoothPermissionScreen> createState() =>
...
class _BluetoothPermissionScreenState extends State<BluetoothPermissionScreen> {
  ...
  void initState() {
    ...
    context
        .read<PermissionBloc>()
        .add(PermissionEventCheckBluetoothPermission());
  }

  Future<void> _handlePermanentlyDenied() async {
    context.showConfirmationDialog(
      title: LocaleKeys
          .core_common_features_permission_presentation_screens_bluetooth_permission_screen_dialog_reAskTitle,
      message: LocaleKeys
          .core_common_features_permission_presentation_screens_bluetooth_permission_screen_dialog_reAskMessage,
  ...
  void _onContinuePressed() {
    context
        .read<PermissionBloc>()
        .add(PermissionEventRequestBluetoothPermission());
  }

  void _onSkipPressed() {
    context
        .read<PermissionBloc>()
        .add(PermissionEventSkipBluetoothPermission());
  }
    ...
    return BlocConsumer<PermissionBloc, PermissionState>(
      listenWhen: (previous, current) {
        final bool statusChanged =
            previous.bluetoothPermissionStatus != current.bluetoothPermissionStatus;
        final bool skippedChanged =
            previous.skippedBluetoothPermissionGranted !=
            current.skippedBluetoothPermissionGranted;
        final bool retryDenied =
            current.bluetoothPermissionStatus ==
                PermissionStatus.permanentlyDenied &&
            previous.bluetoothPermissionStatus ==
                PermissionStatus.permanentlyDenied &&
            previous.isLoading &&
            !current.isLoading;
      ...
      listener: (context, state) async {
        ...
        if (state.bluetoothPermissionStatus == PermissionStatus.granted ||
            state.skippedBluetoothPermissionGranted) {
        ...
        AppLogger.debug(
          'Bluetooth permission status: ${state.bluetoothPermissionStatus}',
        );

        if (state.bluetoothPermissionStatus ==
            PermissionStatus.permanentlyDenied) {
          ...
          body: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                AppText(
                  LocaleKeys
                      .core_common_features_permission_presentation_screens_bluetooth_permission_screen_title,
                ...
                Padding(
                  ...
                  child: AppText(
                    LocaleKeys
                        .core_common_features_permission_presentation_screens_bluetooth_permission_screen_message,
